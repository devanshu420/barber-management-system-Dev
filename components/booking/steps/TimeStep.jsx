...
export function TimeSelection({ serviceDuration, onSelect, shop }) {
  ...
  const convertTo12Hour = (time24) => {
    ...
    return `${hour12.toString().padStart(2, "0")}:${minutesRaw
      .toString()
      .padStart(2, "0")} ${period}`;
  };

  // Normalize any incoming time string to "HH:MM AM/PM"
  const normalizeTo12Hour = (rawTime) => {
    if (!rawTime || typeof rawTime !== "string") return "";

    const time = rawTime.trim().toUpperCase();

    // Format like "09:30 AM" or "9:30 AM"
  ...
  // Generate end time from selected start time
  const generateEndTime = (startTime, duration = 30) => {
    if (!startTime) return "";

    const normalized = normalizeTo12Hour(startTime);
    ...
    return `${endHours.toString().padStart(2, "0")}:${endMinutes} ${endPeriod}`;
  };
  ...
  const isBooked = (time) => {
    ...
    return bookedSlots.some(
      (slot) => normalizeTo12Hour(slot) === normalizedTime
    );
  };
  ...
  useEffect(() => {
    async function fetchBookedSlotsForDate() {
      if (!selectedDate || !shop || !(shop._id || shop.id)) return;

      try {
        const shopId = shop._id || shop.id;
        const dateStr = formatLocalDate(selectedDate);

        console.log("Selected local date:", dateStr);

        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_API_URL}/api/bookings/shop/${shopId}/date/${dateStr}`,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
            },
        ...
        if (response.data.success) {
          ...
          const normalizedBooked = Array.isArray(rawBooked)
            ? rawBooked.map((item) => {
                if (typeof item === "string") {
                  return normalizeTo12Hour(item);
                }

                if (item?.bookingTime?.startTime) {
                  return normalizeTo12Hour(item.bookingTime.startTime);
                }

                if (item?.startTime) {
                  return normalizeTo12Hour(item.startTime);
                }

                if (item?.time) {
                  return normalizeTo12Hour(item.time);
                }
          ...
          console.log("Normalized booked slots:", normalizedBooked);
          setBookedSlots(normalizedBooked);
        } else {
          setBookedSlots([]);
        }
      } catch (err) {
        console.log("No bookings found for this date", err);
        setBookedSlots([]);
      }
  ...
  }, [workingHours, serviceDuration]);

  useEffect(() => {
    console.log("Booked slots state updated:", bookedSlots);
  }, [bookedSlots]);

  // Filter date by working days
  ...
  const handleConfirm = () => {
    if (selectedDate && selectedTime) {
      ...
      onSelect(selectedDate, {
        startTime: normalizeTo12Hour(selectedTime),
        endTime,
      });
    }
  };
  ...
  return (
    <div className="space-y-7 sm:space-y-8">
      ...
      <div className="text-center mb-4 sm:mb-6">
        <div className="inline-flex items-center justify-center w-14 h-14 sm:w-16 sm:h-16 bg-linear-to-br from-emerald-500/25 to-teal-500/25 rounded-2xl mb-4 border border-emerald-500/40 shadow-[0_0_30px_rgba(16,185,129,0.6)]">
      ...
      <Button
        ...
        className="w-full py-3 bg-linear-to-r from-teal-500 via-cyan-500 to-emerald-500 hover:from-teal-400 hover:via-cyan-400 hover:to-emerald-400 disabled:opacity-50 disabled:cursor-not-allowed text-black font-semibold rounded-2xl transition-transform duration-150 hover:scale-[1.02]"
      >
...
//   // 🔹 Fetch shop working hours and (for now) log bookings
//   useEffect(() => {
//     async function fetchShopData() {
//       setLoading(true);
//       setError("");
...
//           // Optional: fetch all bookings for debugging/logging
//           try {
...
//   // 🔹 Fetch booked slots for selected date (UPDATED)
//   useEffect(() => {
//     async function fetchBookedSlotsForDate() {
//       if (!selectedDate || !shop) return;

//       try {
//         // ❌ Pehle: const dateStr = selectedDate.toISOString().split("T")[0];
//         // ✅ Ab: local YYYY-MM-DD string (no timezone issues)
...
//         console.log("dateStr sending to backend:", dateStr);
...
//         console.log("bookedSlots response:", response.data);
