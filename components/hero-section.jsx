...
// ========== HERO (VIDEO BACKGROUND, NO SLIDER) ==========
function HeroSlider() {
  const [notifications, setNotifications] = useState([]);
  const [showNotifications, setShowNotifications] = useState(false);
  const router = useRouter();
  ...
  return (
    ...
    <div className="relative w-full h-screen overflow-hidden shadow-2xl">
      {/* Background video (replaces image slider) */}
      <div className="absolute inset-0 w-full h-full">
        ...
        >
          <source
            src="https://ik.imagekit.io/asdf5690/barber-book/Intro%20Video/Fire_Barber_Shop_Promo_-_ATLMDERON_1080P.mp4"
            type="video/mp4"
          />
        </video>
      </div>

      {/* Gradient Overlay (same as before, just converted to Tailwind gradient) */}
      <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/40 to-black/60" />

      {/* Hero content (UNCHANGED text + animations) */}
      <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-6 z-20">
        <motion.div
          initial={{ opacity: 0, y: 30 }}
          animate={{ opacity: 1, y: 0 }}
