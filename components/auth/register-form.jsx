  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        localStorage.setItem("userRole", formData.role); // 🔹 Store role

        // 🔹 Redirect after 1.5 seconds (use relative URL)
        setTimeout(() => {
          router.push("http://localhost:3000"); // 🔹 Changed from absolute URL
        }, 1500);
      } else {
