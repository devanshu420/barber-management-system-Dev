...
// ============================================
// ROUTE 1: REGISTER SHOP WITH IMAGES
// ============================================

/**
 * POST /api/barber/register-shop
 * Register a new barber shop with images, services, and staff
 * 
 * Form Data:
 ...
 * - images (file[], optional, max 10 files)
...
//         path: `/uploads/shops/${file.filename}`,
//         size: file.size,
...
//   body("shopName").trim().notEmpty().withMessage("Shop name is required").isLength({ min: 2, max: 100 }),
//   body("description").trim().optional().isLength({ max: 500 }),
//   body("address").trim().notEmpty().withMessage("Address is required").isLength({ min: 5, max: 200 }),
//   body("city").trim().notEmpty().withMessage("City is required").isLength({ min: 2, max: 50 }),
//   body("state").trim().notEmpty().withMessage("State is required").isLength({ min: 2, max: 50 }),
//   body("zipCode").trim().optional().matches(/^[0-9]{5,6}$/),
...
//     if (err.code === "FILE_TOO_LARGE") return res.status(400).json({ success: false, message: "File too large (5MB max)" });
//   } else if (err) {
...
// // Register barber shop with images
// router.post(
...
//     try {
//       // Add image paths to req.body so controller can handle them
//       req.body.images = req.files?.map(f => ({
//         filename: f.filename,
//         path: `/uploads/shops/${f.filename}`,
//         size: f.size,
//         uploadedAt: new Date(),
//       })) || [];

//       await registerBarberShop(req, res, next);
//     } catch (err) {
//       if (req.files) req.files.forEach(f => fs.unlink(f.path, () => {}));
//       next(err);
//     }
