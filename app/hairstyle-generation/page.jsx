...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (!apiSuccess || !generatedUrl || typeof generatedUrl !== "string") {
    ...
    setChat((prev) =>
      prev.filter((msg) => msg.id !== userMessage.id || msg.role !== "user")
    );
  } finally {
    setLoading(false);
  }
  ...
  if (checkingAuth) {
    return (
      <main className="flex h-screen items-center justify-center bg-[radial-gradient(circle_at_top_right,rgba(34,211,238,0.16),transparent_22%),radial-gradient(circle_at_bottom_left,rgba(20,184,166,0.14),transparent_20%),linear-gradient(135deg,#020617_0%,#030712_45%,#082f49_100%)] px-4 text-white">
        <div className="rounded-2xl border border-white/10 bg-white/5 px-6 py-4 text-sm text-gray-200 backdrop-blur-xl">
      ...
      <div className="relative z-10 mx-auto flex h-screen w-full max-w-7xl flex-col overflow-hidden px-4 py-4 sm:px-6 lg:px-8">
        <header className="mb-4 flex shrink-0 flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-start gap-3">
            ...
            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-cyan-400 to-teal-300 shadow-lg shadow-cyan-500/30 sm:h-12 sm:w-12">
              <Wand2 className="h-5 w-5 text-slate-950 sm:h-6 sm:w-6" />
            </div>
          ...
          <div className="flex gap-2">
            <button
              type="button"
              onClick={() => setIsMobileSidebarOpen(!isMobileSidebarOpen)}
              className="inline-flex items-center justify-center gap-2 rounded-2xl border border-cyan-400/30 bg-cyan-500/10 px-3 py-2 text-xs font-medium text-cyan-100 shadow-md shadow-cyan-500/10 transition hover:border-cyan-300 hover:bg-cyan-400/15 hover:text-white md:hidden"
            >
              <Sparkles className="h-4 w-4" />
              <span className="hidden xs:inline">Prompts</span>
            </button>
            ...
            <button
              type="button"
              onClick={handleOpenGallery}
              className="inline-flex items-center justify-center gap-2 rounded-2xl border border-cyan-400/30 bg-cyan-500/10 px-3 py-2 text-xs font-medium text-cyan-100 shadow-md shadow-cyan-500/10 transition hover:border-cyan-300 hover:bg-cyan-400/15 hover:text-white sm:px-4 sm:py-2.5 sm:text-sm"
            >
              <GalleryHorizontal className="h-4 w-4" />
              <span>View Photos</span>
            </button>
            ...
            <div className="min-h-0 flex-1 overflow-hidden">
              ...
              >
                <div className="no-scrollbar h-full overflow-y-auto px-2 py-3 sm:px-3 sm:py-4 md:px-4 md:py-5">
                  <div className="space-y-3 sm:space-y-4">
                    {chat.length === 0 && !loading && (
                      <div className="flex flex-col items-center justify-center px-3 py-8 text-center sm:px-4 sm:py-10">
                        <div className="flex h-12 w-12 items-center justify-center rounded-3xl border border-cyan-400/20 bg-cyan-500/10 shadow-inner shadow-cyan-900/30 sm:h-14 sm:w-14">
                          <Wand2 className="h-5 w-5 text-cyan-300 sm:h-6 sm:w-6" />
                        </div>

                        <h2 className="mt-3 text-base font-semibold text-white sm:mt-4 sm:text-lg">
                          Start with a hairstyle idea
                        </h2>
                        <p className="mt-1.5 max-w-xs text-xs leading-5 text-gray-400 sm:mt-2 sm:max-w-md sm:text-sm sm:leading-6">
                          Select a preset from the prompts menu or type your own
                          prompt below, then upload a clear portrait photo.
                        </p>
                      </div>
              ...
              <div
                onMouseDown={() => setIsResizing(true)}
                onTouchStart={() => setIsResizing(true)}
                className={`group relative hidden h-2 shrink-0 cursor-row-resize items-center justify-center border-y border-white/10 sm:flex sm:h-3 ${
                  isResizing ? "bg-cyan-400/10" : "bg-white/5 hover:bg-white/10"
                }`}
              >
                <div className="h-1 w-12 rounded-full bg-white/20 transition group-hover:bg-cyan-300/60 sm:w-16" />
              </div>
              ...
              >
                <div className="no-scrollbar h-full overflow-y-auto px-2 py-2.5 sm:px-3 sm:py-3 md:px-4 md:py-4">
                  <div className="mx-auto w-full max-w-4xl space-y-2.5 sm:space-y-3">
                    {filePreview && (
                      <div className="flex items-center gap-2 rounded-2xl border border-white/10 bg-white/5 p-2.5 sm:gap-3 sm:p-3">
                        <div className="relative h-10 w-10 shrink-0 overflow-hidden rounded-2xl border border-white/10 sm:h-12 sm:w-12">
                          <img
                            src={filePreview}
                            alt="Uploaded preview"
                            className="h-full w-full object-cover"
                    ...
                    {error && (
                      <div className="rounded-2xl border border-red-500/30 bg-red-500/10 px-2.5 py-2 text-[10px] text-red-200 sm:px-3 sm:text-xs">
                        {error}
                      </div>
                    )}
